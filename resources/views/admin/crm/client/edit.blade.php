                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('clients.update', $client->id) }}" method="post" class="form-group"
                                        ...
                                        <div class="col-sm-4">
                                            <div class="form-group">
                                                ...
                                                <input id="contact_no" type="text"
                                                    ...
                                                    name="contact_no" value="{{ old('contact_no', $client->contact_no) }}"
                                        ...
                                        <div class="col-sm-4">
                                            <div class="form-group">
                                                ...
                                                <input id="email" type="email"
                                                    ...
                                                    value="{{ old('email', $client->email) }}" placeholder="Email">
                                                <div class="help-block with-errors"></div>
                                                @error('email')
                                                    <span class="text-danger-error" role="alert">
                                                        <strong>{{ $message }}</strong>
                                                ...
                                                <select id="client_source_id"
                                                    ...
                                                    @foreach ($client_sources as $source)
                                                        <option value="{{ $source->id }}"
                                                            @if (old('client_source_id', $client->client_source_id) == $source->id) selected @endif>
                                                            {{ $source->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select id="business_category_id"
                                                    ...
                                                    @foreach ($business_categories as $category)
                                                        <option value="{{ $category->id }}"
                                                            @if (old('business_category_id', $client->business_category_id) == $category->id) selected @endif>
                                                            {{ $category->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select id="client_status_id"
                                                    ...
                                                    @foreach ($client_statuses as $status)
                                                        <option value="{{ $status->id }}"
                                                            @if (old('client_status_id', $client->client_status_id) == $status->id) selected @endif>
                                                            {{ $status->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select id="interested_in_id"
                                                    ...
                                                    @foreach ($interested_ins as $interest)
                                                        <option value="{{ $interest->id }}"
                                                            @if (old('interested_in_id', $client->interested_in_id) == $interest->id) selected @endif>
                                                            {{ $interest->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                <select id="country_id"
                                                    ...
                                                    @foreach ($countries as $country)
                                                        <option value="{{ $country->id }}"
                                                            @if (old('country_id', $client->country_id) == $country->id) selected @endif>
                                                            {{ $country->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <textarea id="address" rows="10" type="text" class="form-control @error('address') is-invalid @enderror"
                                                    name="address" placeholder="Address">{{ old('address', $client->address) }}</textarea>
                                                <div class="help-block with-errors"></div>
                                                @error('address')
                                                    <span class="text-danger-error" role="alert">
                                                        <strong>{{ $message }}</strong>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <textarea id="note" rows="10" type="text" class="form-control @error('note') is-invalid @enderror"
                                                    name="note" placeholder="Note">{{ old('note', $client->note) }}</textarea>
                                                <div class="help-block with-errors"></div>
                                                @error('note')
                                                    <span class="text-danger-error" role="alert">
                                                        <strong>{{ $message }}</strong>
...
@section('custom-script')
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Initial fetch for states and cities based on current client data
            const initialCountryId = countrySelect.value;
            const initialStateId = "{{ old('state_id', $client->state_id) }}";
            const initialCityId = "{{ old('city_id', $client->city_id) }}";

            if (initialCountryId) {
                fetchStates(initialCountryId, initialStateId, initialCityId);
            }
            ...
            function fetchStates(countryId, stateId = null, cityId = null) {
                if (countryId) {
                    fetch('{{ route('clients.get_states') }}', {
                        ...
                        .then(data => {
                            ...
                            if (stateId) {
                                fetchCities(countryId, stateId, cityId);
                            }
            ...
            function fetchCities(countryId, stateId, cityId = null) {
