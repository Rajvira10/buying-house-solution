                            ...
                            <div class="card-header">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="col-sm-auto">
                                        @if (in_array('investment.create', session('user_permissions')))
                                            <a href="{{ route('investments.create') }}">
                                                <button type="button" class="btn btn-success add-btn">
                                                    <i class="ri-add-line align-bottom me-1"></i> Add Investment
                                                </button>
                                            </a>
                                        @endif
                                        ...
                                        {{-- <a href="{{ route('investments.pending') }}">
                                            <button type="button" class="btn btn-warning"> Pending Investments
                                            </button>
                                        </a> --}}
                                    </div>
                                </div>
                                    ...
                                    <div class="card-body">
                                        <table id="investmentTable" class="table">
                                            <thead class="table-light">
                                                <tr>
                                                    <th>{{ __('Date') }}</th>
                                                    <th>{{ __('Investment No') }}</th>
                                                    <th>{{ __('Investor') }}</th>
                                                    <th>{{ __('Type') }}</th>
                                                    <th>{{ __('Payback Status') }}</th>
                                                    <th>{{ __('Account') }}</th>
                                                    <th>{{ __('Amount') }}</th>
                                                    <th>{{ __('Total Paid') }}</th>
                                                    <th>{{ __('Due') }}</th>
...
@section('custom-script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var dTable = $('#investmentTable').DataTable({
                order: [],
                lengthMenu: [
                    [10, 25, 50, 100, -1],
                    [10, 25, 50, 100, "All"]
