...
@section('title', 'Meeting Statuses')
@section('content')
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="col">
                                        <h4 class="card-title mb-0">Meeting Statuses</h4>
                                    </div>
                                    <div class="col-sm-auto">
                                        @if (in_array('meeting_status.create', session('user_permissions')))
                                            <a href="{{ route('meeting_statuses.create') }}">
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var dTable = $('#businessCategoryTable').DataTable({
                ...
                ajax: {
                    url: "{{ route('meeting_statuses.index') }}",
                    type: "get"
                },
                columns: [{
                        data: 'DT_RowIndex',
        ...
        const deleteMeetingStatus = (id) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('meeting_statuses.destroy') }}",
                        method: 'POST',
                        data: {
                            meeting_status_id: id,
                            _token: '{{ csrf_token() }}'
                        },
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                toaster('Meeting Status Deleted Successfully', 'success');
                            } else {
